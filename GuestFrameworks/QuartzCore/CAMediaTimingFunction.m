...
+ (id)functionWithControlPoints:(float)guest_arg0 :(float)guest_arg1 :(float)guest_arg2 :(float)guest_arg3 {
...
+ (id)uiFunctionWithControlPoints:(const CGPoint *)guest_arg0 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  /* declaration: unhandled type const CGPoint * */ 
  uint64_t host_ret = LC32InvokeHostSelector(self.host_self, _host_cmd, /* parameterToBePassed: unhandled type const CGPoint * */);
...
- (id)initWithControlPoints:(float)guest_arg0 :(float)guest_arg1 :(float)guest_arg2 :(float)guest_arg3 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  double host_arg0 = (double)guest_arg0; 
...
#if 0 // FIXME: has unhandled types
- (void)getControlPointAtIndex:(uint32_t)guest_arg0 values:([2f])guest_arg1 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  uint64_t host_arg0 = (uint64_t)guest_arg0; 
  /* declaration: unhandled type [2f] */ 
  uint64_t host_ret = LC32InvokeHostSelector(self.host_self, _host_cmd, host_arg0, /* parameterToBePassed: unhandled type [2f] */);
  // No post-process for guest_arg0 
  // No post-process for guest_arg1 
  // return void
}
