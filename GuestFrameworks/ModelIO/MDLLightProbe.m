...
- (void)setSphericalHarmonicsCoefficients:(id)guest_arg0 {
...
- (id)initWithReflectiveTexture:(id)guest_arg0 irradianceTexture:(id)guest_arg1 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  uint64_t host_arg0 = [guest_arg0 host_self]; 
...
- (void)generateSphericalHarmonicsFromIrradiance:(unsigned int)guest_arg0 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
...
+ (?)calculateIrradianceGradientUsingSamples:(?)guest_arg0 ofSize:(?)guest_arg1 fromSH:(?)guest_arg2 withLevel:(?)guest_arg3 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
...
#if 0 // FIXME: has unhandled types
+ (CGColor *)sampleSHAt:(id)guest_arg0 usingCoefficients:(unsigned int)guest_arg1 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  uint64_t host_arg0 = [guest_arg0 host_self]; 
...
+ (id)lightProbeWithTextureSize:(int)guest_arg0 forLocation:(id)guest_arg1 lightsToConsider:(id)guest_arg2 objectsToConsider:(id)guest_arg3 reflectiveCubemap:(id)guest_arg4 irradianceCubemap:(id)guest_arg5 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  uint64_t host_arg0 = (uint64_t)guest_arg0; 
