...
- (int32_t)enableBus:(uint32_t)guest_arg0 scope:(uint32_t)guest_arg1 enable:(char)guest_arg2 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
...
- (id)initWithComponentDescription:(AudioComponentDescription)guest_arg0 options:(uint32_t)guest_arg1 error:(id *)guest_arg2 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  /* declaration: unhandled type AudioComponentDescription */ 
  uint64_t host_arg1 = (uint64_t)guest_arg1; 
  uint64_t host_arg2; // id * 
  uint64_t host_ret = LC32InvokeHostSelector(self.host_self, _host_cmd, /* parameterToBePassed: unhandled type AudioComponentDescription */, host_arg1, &host_arg2);
  // No post-process for guest_arg0 
  // No post-process for guest_arg1 
...
#if 0 // FIXME: has unhandled types
- (id)initWithAudioUnit:(OpaqueAudioComponentInstance *)guest_arg0 description:(AudioComponentDescription)guest_arg1 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  /* declaration: unhandled type OpaqueAudioComponentInstance * */ 
  /* declaration: unhandled type AudioComponentDescription */ 
  uint64_t host_ret = LC32InvokeHostSelector(self.host_self, _host_cmd, /* parameterToBePassed: unhandled type OpaqueAudioComponentInstance * */, /* parameterToBePassed: unhandled type AudioComponentDescription */);
  /* postCall: unhandled type OpaqueAudioComponentInstance * */ 
  // No post-process for guest_arg1 
  self.host_self = host_ret; return self;
}
#endif
...
- (char)allocateRenderResourcesAndReturnError:(id *)guest_arg0 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
