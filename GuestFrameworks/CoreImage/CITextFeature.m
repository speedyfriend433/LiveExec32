...
- (id)initWithBounds:(CGRect)guest_arg0 topLeft:(CGPoint)guest_arg1 topRight:(CGPoint)guest_arg2 bottomLeft:(CGPoint)guest_arg3 bottomRight:(CGPoint)guest_arg4 subFeatures:(id)guest_arg5 messageString:(id)guest_arg6 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
