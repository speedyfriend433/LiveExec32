...
- (__GLISharedRec *)getGLIShared {
  ...
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  uint64_t host_ret = LC32InvokeHostSelector(self.host_self, _host_cmd);
  /* returnLine: unhandled type __GLISharedRec * */
}
#endif
...
#if 0 // FIXME: has unhandled types
- (void)loadGLIPlugin:(__GLDPixelFormatRec *)guest_arg0 sharedWithCompute:(char)guest_arg1 {
  printf("DBG: call [%s %s]\n", class_getName(self.class), sel_getName(_cmd));
  static uint64_t _host_cmd;
  if(!_host_cmd) _host_cmd = LC32GetHostSelector(_cmd) | (uint64_t)0 << 63;
  /* declaration: unhandled type __GLDPixelFormatRec * */ 
  uint64_t host_arg1 = (uint64_t)guest_arg1; 
  uint64_t host_ret = LC32InvokeHostSelector(self.host_self, _host_cmd, /* parameterToBePassed: unhandled type __GLDPixelFormatRec * */, host_arg1);
  /* postCall: unhandled type __GLDPixelFormatRec * */ 
  // No post-process for guest_arg1 
  // return void
}
#endif
