...
u32 Dynarmic_map_file(bool isDyld, u32 target, const char *path) {
  ...
  //u32 addr = Dynarmic_direct_mmap(handle, target, 0x1000, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, map, 0);
  ...
  for (uint i = 0; i < header->ncmds; i++, cur += lc->cmdsize) {
    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      u32 mappedAddr = 0;
      if(filesize > 0) {
        mappedAddr = Dynarmic_direct_mmap(handle, target + seg->vmaddr, filesize, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, (void *)(map + seg->fileoff), 0);
        assert(mappedAddr != -1);
      }

      u32 vmMappedAddr = Dynarmic_mmap(handle, target + seg->vmaddr + filesize, seg->vmsize - filesize, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
      if(filesize == 0) {
      ...
      state->__pc += target;
      for (int i = 0; i < 13; i++) {
        handle->jit32->Regs()[i] = state->__r[i];
      }
      handle->jit32->Regs()[13] = state->__sp;
      handle->jit32->Regs()[14] = state->__lr;
      handle->jit32->Regs()[15] = state->__pc;
      handle->jit32->SetCpsr(state->__cpsr);
    }
  }
...
u32 prependString(u32& address, const char* fmt, ...) {
  ...
  address -= len + 1;
  Dynarmic_mem_1write(handle, address, len, buffer);
  return address;
}
...
int main(int argc, char* argv[], char* envp[]) {
  ...
  handle = Dynarmic_nativeInitialize();
  ...
  if (getuid() == 0) {
    ...
    //handle->fs->addMountpoint("/rootfs", rootPath);
    handle->fs->addMountpoint("/", rootPath);
  }

  // commpage 0xffff4000+0x1000
  u32 commpage = Dynarmic_mmap(handle, 0xffff4000, 0x1000, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
  handle->ucb->MemoryWrite16(commpage + 0x1E, 3); // version
  handle->ucb->MemoryWrite32(commpage + 0x20, 0x9000);
  handle->ucb->MemoryWrite8(commpage + 0x22, 1); // number of CPUs
  handle->ucb->MemoryWrite8(commpage + 0x24, DYN_PAGE_BITS); // 32-bit page shift
  handle->ucb->MemoryWrite16(commpage + 0x26, 128); // cache line size, 32? 64?
  //handle->ucb->MemoryWrite32(commpage + 0x28, 128); // sched count
  handle->ucb->MemoryWrite8(commpage + 0x34, 1); // active CPU
  handle->ucb->MemoryWrite8(commpage + 0x35, 1); // physical CPU
  handle->ucb->MemoryWrite8(commpage + 0x36, 1); // logical CPU
  handle->ucb->MemoryWrite8(commpage + 0x37, DYN_PAGE_BITS); // kernel page shift
  handle->ucb->MemoryWrite32(commpage + 0x38, 0x40000000u); // max memory size, 1GB
  // TODO: mach time stuff
  //handle->ucb->MemoryWrite64(commpage + 0x40, 0x4141414141414141); // FIXME
  //handle->ucb->MemoryWrite64(commpage + 0x80, 0x41414141); // FIXME
  handle->ucb->MemoryWrite64(commpage + 0x84, 1); // dev firmware
  
  // allocate stack guards and stack buffer for dyld
  u32 dyldStackGuardStart = Dynarmic_mmap(handle, 0x80000000, 0x1000, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  u32 dyldStack = Dynarmic_mmap(handle, 0x80000000, 0xff000, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  Dynarmic_mmap(handle, 0x80000000, 0x1000, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  for (int i = 0; i < sizeof(guest_apple)/sizeof(u32); i++) {
    handle->ucb->MemoryWrite32(dyldStackPtr -= sizeof(u32), guest_apple[i]);
  }

  // write envp
  for (int i = 0; i < sizeof(guest_envp)/sizeof(u32); i++) {
    handle->ucb->MemoryWrite32(dyldStackPtr -= sizeof(u32), guest_envp[i]);
  }

  // write argv and argc
  for (int i = argc; i >= 0; i--) {
    handle->ucb->MemoryWrite32(dyldStackPtr -= sizeof(u32), guest_argv[i]);
  }

  // write main executable base
  handle->ucb->MemoryWrite32(dyldStackPtr -= sizeof(u32), execAddr);

  printf("stack ptr now 0x%x\n", dyldStackPtr);

  Dynarmic_reg_1write(handle, 13, dyldStackPtr);
  Dynarmic_emu_1start(handle, handle->jit32->Regs()[15]);
  Dynarmic_nativeDestroy(handle);
}
